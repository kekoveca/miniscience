...
public:
    ...
    void move(double tau) {
        x += vx * sin(time * 16 * M_PI) / 25;
        y += vy * cos(time * 16 * M_PI) / 25;
        z += vz * sin(time * 2 * M_PI);
    }

    // Эволюцию скалярной функции 
    void calcScalarField(double tau) {
        double r = sqrt(x * x + y * y + z * z);
        complex<double> cmplx_unit(0, 1);
        smth += (50 / r) * exp(cmplx_unit * (40 * time - 1 * r)).real(); 
    }

    void timeStep(double tau) {
        calcScalarField(tau);
        move(tau);
        time += tau;
    }
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            // Координаты заберём из gmsh, вычтем 0 координат. Почему-то центр бублика не в центре координат
            // Заменим Y на Z, чтобы бублик лежал в плоскости XY
            double pointX = nodesCoords[i*3] - 65;
            double pointZ = nodesCoords[i*3 + 1] - 25;
            double pointY = nodesCoords[i*3 + 2] - 65;
            // Сферические координаты и скалярная функция от них
            double r = sqrt(pointX * pointX + pointY * pointY + pointZ * pointZ);
            double phi = atan2(pointY, pointX);
            double theta = atan(sqrt(pointX * pointX + pointY * pointY) / pointZ);
            double smth = r * cos(theta) + 10 * sin(phi + M_PI);
            double vx = 45 * cos(phi);
            double vy = - 45 * sin(phi);
            double vz = 10 * cos(theta);
    ...
    void doTimeStep(double tau) {
        for(unsigned int i = 0; i < nodes.size(); i++) {
            nodes[i].timeStep(tau); // один шаг по времени
        }
    }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "model3d-step-" + std::to_string(snap_number) + ".vtu";
